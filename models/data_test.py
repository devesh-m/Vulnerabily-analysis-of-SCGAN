...
class faces_super(data.Dataset):
    def __init__(self, datasets, transform):
        ...
        def resolve_test_dir(folder_name):
            direct = os.path.join(BASE_DIR, 'imgs_test', folder_name)
            nested = os.path.join(BASE_DIR, 'dataset', 'imgs_test', folder_name)
            if os.path.exists(direct):
                return direct
            return nested

        if dataset == 'LS3D':
            img_path = resolve_test_dir('LS3D-w_balanced')
            list_name = (glob(os.path.join(img_path, "*.png")))
            list_name.sort()
            for filename in list_name:
                self.img_list.append(filename)
        if dataset == 'FFHQ':
            img_path = resolve_test_dir('FFHQ')
            list_name = (glob(os.path.join(img_path, "*.png")))
            list_name.sort()
            for filename in list_name:
                self.img_list.append(filename)
        if dataset == 'Widerface':
            img_path = resolve_test_dir('Widerface')
            list_name = (glob(os.path.join(img_path, "*.png")))
            list_name.sort()
            for filename in list_name:
                self.img_list.append(filename)
        if dataset == 'WebFace':
            img_path = resolve_test_dir('WebFace')
            list_name = (glob(os.path.join(img_path, "*.png")))
            list_name.sort()
            for filename in list_name:
                self.img_list.append(filename)
